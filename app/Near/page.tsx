...
export default function HomeAndNearLayout() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const generatedLink = `upi://pay?pa=${content}&pn=RecipientName&am=Amount&cu=INR&tn=Payment%20for%20Order%20#12345`;
    ...
    <div className="h-screen w-full grid grid-cols-12 bg-zinc-800 grid-rows-12">
      ...
      {paymentLink && (
        <div className="m-4 p-4 bg-white rounded-lg">
          <p className="text-black">Click the link below to make a payment:</p>
          <a href={paymentLink} className="text-blue-500" target="_blank" rel="noopener noreferrer">
            {paymentLink}
          </a>
        </div>
      )}
